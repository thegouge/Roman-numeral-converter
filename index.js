...
export function convertToRoman(num) {
  ...
  const decim = num.toString().split("").map((digit, i, arr) => {
    ...
    if(valueIndex < 0) {
      if(digit / 8 === 1 || digit/8 === 10 || digit/8 === 100) {
        valueIndex = decimValue.indexOf(digit/8);
              return romanValue[valueIndex - 2] + romanValue[valueIndex] + romanValue[valueIndex] + romanValue[valueIndex];
      }
      ...
      if(valueIndex < 0) {
        ...
        if(valueIndex < 0) {
          ...
          if(valueIndex < 0) {
            ...
            return romanValue[valueIndex - 2] + romanValue[valueIndex] + romanValue[valueIndex];
          }
          return romanValue[valueIndex - 2] + romanValue[valueIndex];
        }
        return romanValue[valueIndex] + romanValue[valueIndex] + romanValue[valueIndex];
      }
